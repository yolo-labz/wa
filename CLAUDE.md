...
| Area | Choice | Why |
|---|---|---|
| Language | **Go** — minimum **1.22** at the toolchain, dev host pinned in `go.mod` (currently `go 1.26.5`). Future bumps must update CLAUDE.md, `flake.nix`, and the GitHub Actions matrix in lockstep. | whatsmeow is the only production-grade WA library in 2026; no Rust/Python alternative exists |
...
| Governance toolchain | **`golangci-lint` v1.62+ with `depguard` enforcing the `internal/{domain,app}` ↛ whatsmeow boundary**, `git-cliff` for changelog, `Renovate` for deps, `lefthook` for pre-commit/commit-msg/pre-push, OSV-Scanner V2 in CI (invokes `govulncheck` internally — see the research note on dropping the standalone run). Configs committed under `001-research-bootstrap`. | depguard is the single most important line of YAML in the repo — it enforces the hexagonal invariant from outside the language. |
...
2. **Rate limiter** as non-overridable middleware between use case and adapter. Ordinary sends are limited to 2/second with burst 2 and 30/minute with burst 30. They have no global daily, per-recipient daily, or unique-new-recipient daily cap. There is no `--force`, bypass, or disable knob. Separate hard refusals remain: ≤5 group creations/day, ≤50 participant adds/day, and no broadcast lists ever.
3. **Warmup** auto-engaged on a fresh session DB: 25 % of both ordinary-send short-window rates and bursts while session age is <7 days, 50 % from age 7 days through <14 days, and 100 % from age 14 days onward; every scaled burst remains at least one.
