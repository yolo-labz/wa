...
| T2 | Malicious allowlisted contact | A trusted contact's account is compromised and used to drive the assistant | Per-action allowlist (`read` ≠ `send` ≠ `group.add`); non-overridable 2/second and 30/minute anti-burst limits; manual review of the append-only audit log; fast `wa allow remove <jid>` |
