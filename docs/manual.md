...
- **Watchdog + joined worker**: a 120 s watchdog bounds the caller's wait across send + completion + verification, and `Close()` joins an in-flight repair (aborted via shutdown). If an inner whatsmeow call is ever stuck on a non-context-aware upstream mutex, the caller still gets its timeout — the collection stays excluded until the worker actually exits, so two repairs can never overlap.
