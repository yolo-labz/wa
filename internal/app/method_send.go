...
// ErrRecipientMoved). It blocks ONLY when the recipient is a phone-number
// JID (`@s.whatsapp.net`) that the server confirms has no WhatsApp
// account, or that the server routes under a different JID.
...
func (d *Dispatcher) ensureOnWhatsApp(ctx context.Context, jid domain.JID) error {
	...
	canonical, on, err := d.onWhatsApp.IsOnWhatsApp(ctx, jid.User())
	if err != nil {
		return nil
	}
	if !on {
	...
	// Registered, but not under the JID we were handed. Sending to the
	// requested one fails later with an opaque error, so refuse here and
	// name the JID that works. Compared on the user segment only: the
	// server answers a phone query with a phone JID, and a same-number
	// reply must not be read as a move. Issue #354.
	if !canonical.IsZero() && canonical.IsUser() && canonical.User() != jid.User() {
		return RecipientMoved(jid.String(), canonical.String())
	}
	return nil
}
