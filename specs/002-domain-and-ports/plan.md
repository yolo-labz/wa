...
**Scale/Scope**: ~900 lines of Go across **~25 files** (correcting an earlier underestimate of ~12). The actual breakdown: 11 source files + 9 test files in `internal/domain/` (20), 1 file in `internal/app/ports.go` (1), ~8 files in `internal/app/porttest/` (suite.go + 7 per-port files), 3 files in `internal/adapters/secondary/memory/` (adapter.go, clock.go, adapter_test.go), totalling ~32 files when counting tests, ~22 source files when not. ~30 test functions in `porttest/` plus ~76 in `internal/domain/` = ~106 total. One maintainer, one workstation. The deliverable is library code that downstream features depend on, not user-visible behaviour.
...
| III | Safety first, no `--force` ever (NON-NEGOTIABLE) | **PARTIAL — JUSTIFIED** | The `Allowlist` and `Action` domain types land here (the data structures). The rate limiter middleware, warmup ramp, audit log writer, and `<channel source="wa">` tag wrapper land in feature 004 because they all need the daemon's clock, persistence, and IPC. This split is the correct hexagonal layering: data vs orchestration. The constitution principle is satisfied because *every component the spec mandates is on disk before the first `Send` call leaves `wad`* — and `wad` does not exist yet. **Reordering boundary**: pulling the rate limiter into feature 002 would require a constitution amendment because Constitution Principle III's intent is "rate limiter as non-overridable middleware between use case and adapter" — the middleware is a `wad` composition-time concern, not a domain concern. Any PR that adds rate-limiting to `internal/domain/` or `internal/app/` MUST be accompanied by a constitution amendment with version bump explaining why the reordering is correct. |
...
The directory tree above is the canonical layout. It MUST exactly match `data-model.md §"Package layout"` (11 source files in `internal/domain/`) and `contracts/ports.md §"Package, imports, header"` (1 file `internal/app/ports.go`) and the contract suite in `internal/app/porttest/` (8 files: `suite.go` + `sender.go` + `stream.go` + `directory.go` + `groups.go` + `session.go` + `allowlist.go` + `audit.go`). The in-memory adapter contributes 3 files. Source-file total: 11 + 1 + 8 + 3 = **23**. With the 9 domain `_test.go` files and the 1 in-memory `adapter_test.go`, the test-file total is 10. Grand total: **33 files**. The "~25 files" mentioned in Technical Context is the source-file count without per-domain `_test.go` files; the "~33 files" includes them. Both numbers are correct under their respective definitions; the discrepancy with the original "~12 files" estimate is recorded here as a deliberate correction (CHK043, CHK044 in `checklists/architecture.md`).
