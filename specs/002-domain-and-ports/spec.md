...
- **SC-002**: `go test ./...` on a fresh clone with no network and no environment variables passes and exercises at least one test per port method via the contract suite. **Performance target**: under 5 seconds — this is an *assumption to measure at first `/speckit:implement` run*, not a current observation. If the measured time exceeds 5 seconds, the spec amendment is to revise SC-002 to the measured value; if it exceeds 10 seconds, the `lefthook` `pre-push` test step must be moved to CI only.
