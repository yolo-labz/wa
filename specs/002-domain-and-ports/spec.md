...
- **SC-002**: `go test ./...` on a fresh clone with no network and no environment variables passes and exercises at least one test per port method via the contract suite. **Performance target**: under 5 seconds. **Measured at first `/speckit:implement` run (2026-04-07)**: ~1.5 s wall-time for `go test -race -count=1 ./...` on darwin-arm64 — comfortably under the 5 s cap, so `lefthook` `pre-push` remains appropriate.
