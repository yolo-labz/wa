...
- **What happens when feature 003 needs to add an eighth port?** The feature must amend this spec, the constitution principle that mandates "exactly seven ports", and the contract test suite together — a single port addition cannot land in isolation.
...
- **FR-007**: The repository MUST contain an `app` package with a `ports.go` file declaring exactly seven exported interface types: `MessageSender`, `EventStream`, `ContactDirectory`, `GroupManager`, `SessionStore`, `Allowlist`, and `AuditLog`. The names and shapes MUST match the locked decisions in `CLAUDE.md` §"Ports".
